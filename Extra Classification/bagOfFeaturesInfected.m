%bagOfFeaturesInfected Create bag of visual features.
%   bag = bagOfFeaturesInfected(imds) returns a bag of visual features. imds is
...
%   algorithm.
%
%   bag = bagOfFeaturesInfected(imds,'CustomExtractor',extractorFcn) returns a
...
%
%     <a href="matlab:helpview(fullfile(docroot,'toolbox','vision','vision.map'),'bagOfFeaturesInfectedCustomFeatures')">Learn more about writing a custom feature extractor.</a>
%
%     <a href="matlab:bagOfFeaturesInfected.createExtractorTemplate">Open a custom feature extractor template.</a>
%
%   bag = bagOfFeaturesInfected(...,Name,Value) specifies additional
...
%                       Default: true
%
%   bagOfFeaturesInfected methods:
%      encode - Create a feature vector, a histogram of visual word occurrences
%
%   bagOfFeaturesInfected read-only properties:
...
%   
%   - bagOfFeaturesInfected supports parallel computing using multiple MATLAB
...
%   bag = bagOfFeaturesInfected(trainingSet); % bag creation can take a few minutes
%
...
%   extractor = @examplebagOfFeaturesInfectedExtractor;
%   bag = bagOfFeaturesInfected(imds,'CustomExtractor',extractor) 
%
%   % <a href="matlab:edit('examplebagOfFeaturesInfectedExtractor')">Open examplebagOfFeaturesInfectedExtractor for more details.</a>
%
...
classdef bagOfFeaturesInfected < vision.internal.EnforceScalarHandle & matlab.mixin.CustomDisplay
    ...
    methods (Access = public)
        ...
        %------------------------------------------------------------------
        % Constructor
        function this = bagOfFeaturesInfected(varargin)

            if nargin == 0
                
                % Just return empty object with all default configuration
                % settings.
                d = bagOfFeaturesInfected.getDefaultSettings;
                this = this.setParams(d);
                this.VocabularySize = 0; % this is actually an empty bag
                
            else
                % parse the inputs
                [imgSets, params] = bagOfFeaturesInfected.parseInputs(varargin{:});

                this = this.setParams(params);
                                
                % Point of Attack --> Shayan!
                if ((mod(this.VocabularySize,2) == 0) && (this.VocabularySize >= 100))
        ...
        function [featureVector, varargout] = encode(this, in, varargin)
            ...
            %  be grayscale or truecolor. bag is the bagOfFeaturesInfected object.
            ...
            %  bag = bagOfFeaturesInfected(trainingSet);
            % 
            ...
            params = bagOfFeaturesInfected.parseEncodeInputs(in, varargin{:});
            ...
            if ~isnumeric(in)        
                ...
                bagOfFeaturesInfected.printImageSetDescription(printer, in);

                if bagOfFeaturesInfected.isImageSet(in)
                    numImages = sum([in.Count]);
                    
                    featureVector = bagOfFeaturesInfected.allocateFeatureVector(numImages, this.VocabularySize, params.SparseOutput);
    ...
    methods (Hidden, Access = protected)
        ...
        function [groupedDescriptors, groupedScores] = extractDescriptorsFromImageDS(this, imds, params)
            ...
            numCategories      = bagOfFeaturesInfected.getNumCategories(imds);
        ...
        function [descriptors, scores] = extractDescriptors(this, imgSets, params, printer)
            ...
            bagOfFeaturesInfected.printImageSetDescription(printer,imgSets);
             
            this.printPointSelectionInfo(printer);
            
            if bagOfFeaturesInfected.isImageSet(imgSets)
        ...
        function [features, varargout] = encodeScalarImageSet(this, imgSet, params)
            ...
            features = bagOfFeaturesInfected.allocateFeatureVector(numImages, this.VocabularySize, params.SparseOutput); 
            words    = bagOfFeaturesInfected.allocateVisualWords(numImages);
        ...
        function clusterCenters = createVocabulary(this, descriptors, varargin)
            ...
            params = bagOfFeaturesInfected.parseCreateVocabularyInputs(varargin{:});
        ...
        function [features, featureMetrics, varargout] = invokeCustomExtractor(this, img, extractor)
            ...
            featureLength = bagOfFeaturesInfected.checkCustomExtractorSizes(features, featureMetrics, varargout{:});
    ...
    methods (Hidden, Static, Access = protected)
        ...
        function [tf, ps] = checkAndAssignPointSelection(pointSelection)
            ...
            ps = validatestring(pointSelection,...
                bagOfFeaturesInfected.ValidPointSelectionOptions, mfilename, 'PointSelection');
        ...
        function featureLength = validateCustomExtractor(imgSet, extractor) 
            ...
            if bagOfFeaturesInfected.isImageSet(imgSet)
            ...
            featureLength = bagOfFeaturesInfected.checkCustomExtractorSizes(features, metrics);
        ...
        function [imgSets, params] = parseInputs(varargin)                     
            ...
            parser = inputParser;
            parser.addRequired('imds', @(x)bagOfFeaturesInfected.checkImageSet(x,mfilename));
            
            parser.addParameter('VocabularySize',    d.VocabularySize,    @bagOfFeaturesInfected.checkVocabularySize);
            parser.addParameter('StrongestFeatures', d.StrongestFeatures, @bagOfFeaturesInfected.checkStrongestFeatures);
            parser.addParameter('PointSelection',    d.PointSelection,    @bagOfFeaturesInfected.checkAndAssignPointSelection);
            parser.addParameter('GridStep',          d.GridStep,          @bagOfFeaturesInfected.checkGridStep);
            parser.addParameter('BlockWidth',        d.BlockWidth,        @bagOfFeaturesInfected.checkBlockWidth);
            ...
            if bagOfFeaturesInfected.isImageSet(parser.Results.imds)                
            ...
            params.ExtractorOutputsLocations = bagOfFeaturesInfected.checkCustomExtractor(parser.Results.CustomExtractor);
            ...
            if wasSpecified('CustomExtractor')
                ...
                featureLength = bagOfFeaturesInfected.validateCustomExtractor(imgSets,params.CustomExtractor);
            ...
            [~, params.PointSelection] = ...
                bagOfFeaturesInfected.checkAndAssignPointSelection(parser.Results.PointSelection);
        ...
        function params = parseEncodeInputs(varargin)            
            ...
            defaults = bagOfFeaturesInfected.getDefaultSettings;
            ...
            bagOfFeaturesInfected.validateEncodeInput(parser.Results.imds);
        ...
        function printImageSetDescription(printer,imgSets)
            cats = bagOfFeaturesInfected.getCategoryStrings(imgSets);
    ...
    methods (Hidden, Static)
        ...
        function this = loadobj(s)
            % reconstruct bagOfFeaturesInfected                       
            this = bagOfFeaturesInfected();    
            
            if all(isfield(s,{'CustomExtractor', 'Upright', 'ExtractorOutputsLocations'})) % added in R2015a                                                                 
                
                bagOfFeaturesInfected.checkCustomExtractorOnLoad(s.CustomExtractor);                               
